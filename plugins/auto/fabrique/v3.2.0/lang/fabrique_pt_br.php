...
Atenção, de outra forma, o plugin será criado em <code>tmp/cache/@dir_cache@</code>;  esta pasta é excluída quando se esvazia o cache.',
	'c_fabrique_git' => 'Uau, é muito fácil!',
	'c_fabrique_git_texte' => 'Certamente, você vai gostar de produzir um plugin que gerencie um ou mais objetos editoriais.
Ainda bem!<br /><br />
...
É aconselhável que você compreenda previamente o funcionamento dos plugins, do SPIP e dos seus templates, e se desejar criar objetos editoriais, o funcionamento dos pipelines, autorizações, formulários. Este plugin pode, contudo, servir para estudar o código gerado em função das opções que você selecionar.',
	...
	'chaine_titre_page_objets' => 'Os @objets@',
	'chaine_titre_page_objets_feminin' => 'Os @objets@',
...
Esta parte pode evoluir ou desaparecer em futuras versões.',
	...
	'label_champ_date_publication_ignore' => 'Ignorar o campo data de publicação?',
	'label_champ_date_publication_ignore_case' => 'Não gerenciar o campo data de publicação, mesmo que um campo <code>date</code> esteja presenta na lista de campos do objeto.',
...
Além disso, se a sua tabela contém já linhas com dados não inteiros na chave primária, ou com zeros à esquerda (0123), esses dados não poderão ser lidos pelo SPIP porque ele aplica a função intval (força um valor a ser um número inteiro) automaticamente a todo o campo prefixado por id_ e à chave primária de um objeto editorial.',
	...
	'label_liaison_directe_explication' => 'Permite atribuir este objeto a outro objeto superior. Este objeto incluirá na sua tabela a chave primária do objeto superior selecionado aqui. Estes objetos serão listados na ficha do objeto superior. Deve existir uma entrada para o objeto superior selecionado aqui.',
	'label_libelle' => 'Texto',
...
<code>tradutor,Tradutor</code>',
	...
	'label_saisies_mode' => 'Modo de entradas',
	'label_saisies_mode_explication' => 'As entradas dos formulários podem ser inseridas quer na forma de tags no HTML, quer declaradas em PHP.',
	...
	'legend_autorisations_explication' => 'Permite definir para certas ações sobre o objeto editorial, tais como a sua modificação, os testes de autorização que serão realizados. Eles podem depender do status do autor, da seção a que pertence o objeto (se ele conta com o campo id_rubrique), ou ainda do status do próprio objeto (se já está publicado ou não). A autorização de <code>criar</code> não tem todas as opções.',
	'legend_chaines_langues' => 'Cadeias de idioma',
	'legend_champs' => 'Campos',
	'legend_champs_speciaux' => 'Campos especiais',
	...
	'plugin_mis_a_jour' => 'A configuração do plugin foi gravada',
