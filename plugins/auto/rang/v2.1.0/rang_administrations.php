...
/**
 * Maj 1.0.2 : activer les articles par défaut
 **/
function rang_maj_1_0_2() {
	include_spip('inc/config');

	if (!lire_config('rang')) {
		ecrire_config('rang',
			[
				'objets'   => ['spip_articles'],
				'rang_max' => 'on',
			]);
	}
}
...
/**
 * Maj 1.0.3 : correction du bug introduit dans maj_1.0.2() : il faut aussi créer le champ 'rang' dans la table spip_articles
 **/
function rang_maj_1_0_3() {
	include_spip('inc/config');
	include_spip('inc/rang');
	$objets_actifs = lire_config('rang/objets', []);
	$champs_table = sql_showtable('spip_articles');

	if (
		in_array('spip_articles', $objets_actifs)
		and !isset($champs_table['field']['rang'])
	) {
		rang_creer_champs(['spip_articles']);
	}
}
