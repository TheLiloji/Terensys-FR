...
function compresseur_ecrire_balise_css_dist(&$flux, $pos, $src, $comments = '', $media = '') {
	...
	$comments .= '<' . "?php header('Link: <' . url_de_base() . (\SpipLeague\Component\Kernel\app()->isBackOffice() ? \SpipLeague\Component\Kernel\param('spip.routes.back_office') : '') . '$src>;rel=\"preload\";as=\"style\"'); ?>";
	$flux = substr_replace($flux, $comments, $pos, 0);
