	...
	while (preg_match(BALISE_INCLURE, $texte, $match)) {
		...
		if ($fichier) {
			...
			foreach (debug_backtrace() as $trace) {
				if (
					str_starts_with($trace['function'] ?? '', 'public_phraser_html')
					&& isset($trace['args'][3]['sourcefile'])
				) {
					$lieu = $trace['args'][3]['sourcefile'];
					break;
				}
			}
			trigger_deprecation('spip', '4.4', 'Using "%s" in "%s" is deprecated , use "%s" instead', '<INCLURE(' . $fichier . ')>', $lieu, '<INCLURE{fond=...}>');
		}
		...
		$count = 0;
		$texte = preg_replace(',^</INCLU[DR]E>,', '', $texte, -1, $count);
		if ($count) {
			$lieu = '?';
			foreach (debug_backtrace() as $trace) {
				if (
			...
			trigger_deprecation('spip', '4.4', 'Using "%s" in "%s" is deprecated , remove it.', '</INCLURE>', $lieu);
		}
		$result[] = $champ;
	}
...
function phraser_idiomes($texte, $ligne, $result) {
	while (preg_match(BALISE_IDIOMES, $texte, $match)) {
		...
		if ($chaine != $match[3]) {
			trigger_deprecation('spip', '4.4', sprintf('La chaine de langue « %s » a été basculée en minuscule. À partir de SPIP 5.0, les chaînes de langues seront sensibles à la casse.', $match[3]));
		}
...
/**
 * Mémorise un morceau de code du squelette en le remplaçant par un placeholder, ou reinjecte les morceaux de codes à la place des placeholders.
...
function phraser_boucle_placeholder(&$champ, $boucle_placeholder = null, $boucle = null) {
	trigger_deprecation('spip', '4.4', 'Using "%s" is deprecated, use "%s" or "%s" instead', 'phraser_boucle_placeholder', 'phraser_placeholder_memoriser', 'phraser_placeholder_reinjecter');
...
function public_placeholder_generer(string $nom_structure, &$boucle_ou_champ, string $placeholder_pattern, int $nb_lignes, bool $force_balise_etendue = false): string {
	if ($nb_lignes || $force_balise_etendue) {
...
function public_generer_boucle_placeholder($id_boucle, &$boucle, $boucle_placeholder, $nb_lignes) {
	trigger_deprecation('spip', '4.4', 'Using "%s" is deprecated, use "%s" instead', 'public_generer_boucle_placeholder', 'public_placeholder_generer');
...
function public_phraser_html_dist(
	...
	while ($boucle = public_trouver_premiere_boucle($texte, $id_parent, $descr, $pos_debut_texte)) {
		...
		$placeholder = public_placeholder_generer($id_boucle, $boucles[$id_boucle], $boucle_placeholder, $ligne_suite - $ligne_debut_texte, true);
