...
function f_boite_infos($flux) {
	...
	$args['espace_prive'] = 1;
	$flux['data'] .= recuperer_fond("prive/objets/infos/$type", $args, ['ajax' => true]);
...
function mise_a_jour_affiche_milieu($flux) {
	...
	include_spip('inc/presentation_mini');
	// il existe une mise à jour identifiée ?
	if (!info_maj_spip()) {
		return $flux;
	}

	// vérifions qu’elle est mineure
